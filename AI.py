...
chinese_model_path = ".\model\CN\G_953000.pth"
...
class AIClient:
    ...
    def InputMessage(self,input_str):

            if input_str.startswith("change"):
                try:
                    id = int(input_str[6:])
                    return
                    # 进行相应的操作
            ...
            resp = self.api.send_message(input_str)
            ...
            generateSound("[ZH]"+answer+"[ZH]", self.id, self.model_id)
...
#             resp = api.send_message(input_str)
