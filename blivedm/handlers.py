...
class BaseHandler(HandlerInterface):
    ...
    def __super_red_packet_callback(self, client: client_.BLiveClient, command: dict):
        return self._on_super_red_packet_callback(client, models.redPacketMessage.from_command(command['data']))
    def __click_like(self, client: client_.BLiveClient, command: dict):
        return self._on_click_like_callback(client, models.likeClickMessage.from_command(command['data']))
    # cmd -> 处理回调
    _CMD_CALLBACK_DICT: Dict[
        ...
        'POPULARITY_RED_POCKET_NEW':__super_red_packet_callback,

        'LIKE_INFO_V3_CLICK':__click_like,
    }
    # 忽略其他常见cmd
    ...
    async def _on_super_chat_delete(self, client: client_.BLiveClient, message: models.SuperChatDeleteMessage):
        ...
        删除醒目留言
        """
    async def  _on_super_red_packet_callback(self, client: client_.BLiveClient, message: models.redPacketMessage):
         """
        红包
        """
    async def  _on_click_like_callback(self, client: client_.BLiveClient, message: models.redPacketMessage):
        """
        点赞
        """    